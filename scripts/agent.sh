...
mkdir -p "$BRAIN_DIR/memories" "$BRAIN_DIR/memories/context" "$BRAIN_DIR/logs" "$AGENT_LEARNING_DIR" "$AGENTS_DIR"
echo "Hermes brain directory and logs initialized."
...
# --- LOCAL FAILSAFE ---
if ! $PROVIDER_SUCCESS; then
    echo "All cloud providers failed or had no API keys. Falling back to Ollama (phone:${OLLAMA_HOST_IP:-192.168.1.42}:11434)..."
    export OLLAMA_HOST="http://${OLLAMA_HOST_IP:-192.168.1.42}:11434"
    OLLAMA_URL="http://${OLLAMA_HOST_IP:-192.168.1.42}:11434/api/generate"
